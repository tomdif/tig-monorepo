...
expansion, followed by coordinate refinement over feasible per-battery actions. \
Hyperparameters: horizon_steps, min_window_std, jump_z_threshold, \
dp_soc_levels, dp_action_levels. profit_floor_shrink is retained only for \
serde compatibility."
    );
}
...
pub fn policy(
    ...
    Ok(expand_to_feasibility_limit(challenge, state, &action))
}
